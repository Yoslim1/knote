...
**Knote** (無所得) is a Zen Buddhist term meaning *acting without grasping for a result*, without aim, without striving for gain.

Which makes it a slightly absurd name for a productivity app. That's the point.

Most productivity apps want you to optimize every minute, defend every streak, and turn your one short life into a backlog. Knote goes the other way: work out the few things that actually matter today, do them, and then (the genuinely hard part) let the rest go. It's a tool for focusing, not for filling. The aim is fewer open tabs in your head, not more.

And then, ideally, you put the phone down. It's an app that's quietly rooting for you to use it less.

Everything you write, track, and plan stays **on your device**. Knote ships with **no `INTERNET` permission at all**, so it physically cannot send your data anywhere. Not because we promise not to, but because the option was never built.

---

## 🌱 What Knote won't do

Some things are missing on purpose. Knote will never:
...
Every release on GitHub is signed with the project release key, and each artifact ships with a **SHA-256** checksum on the Releases page.
...
Knote is free and open-source software. Bundled third-party libraries and their licenses are viewable in-app under *Settings → Licenses*.

---

## 📄 License

Knote is licensed under the **GNU General Public License v3.0**.
